/* ===========================
   Premier MOT — Public Booking Page
   =========================== */

// ===========================
// SHARED BOOKING UTILITIES
// ===========================
const BOOKINGS_KEY = 'premier_bookings';
...
function genBookingRef() {
  const n = (getBookings().length + 1).toString().padStart(4,'0');
  return 'PMB-' + n + '-' + Date.now().toString().slice(-3);
}
...
function getAvailableSlots(dateStr) {
  ...
  return slots.map(t => {
    const count = existing.filter(b => b.date === dateStr && b.time === t && b.status !== 'Cancelled').length;
...
if (document.getElementById('publicBookingPage')) {
  ...
  document.getElementById('nextStep1')?.addEventListener('click', () => { if (!bkData.service) return; goStep(2); });
  ...
  document.getElementById('confirmBookingBtn')?.addEventListener('click', () => {
    ...
    const booking = { id: Date.now(), ref, ...bkData, status: 'Confirmed', createdAt: new Date().toISOString() };
        ...
        return `
          <div class="cal-day${isToday?' cal-today':''}">
            <div class="cal-day-header">
              <span class="cal-day-name">${d.toLocaleDateString('en-GB',{weekday:'short'})}</span>
              <span class="cal-day-date">${d.getDate()}</span>
            </div>
            <div class="cal-day-body">
              ${dayBookings.length === 0 ? '<div class="cal-empty">Free</div>' :
                dayBookings.sort((a,b)=>a.time.localeCompare(b.time)).map(b=>`
                  <div class="cal-booking" onclick="viewBookingModal(${b.id})">
                    <div class="cal-booking-time">${b.time}</div>
                    <div class="cal-booking-name">${esc(b.name.split(' ')[0])}</div>
                    <div class="cal-booking-service">${esc(b.service)}</div>
                  </div>`).join('')
              }
            </div>
          </div>`;
...
function updateBookingStatus(id, status) {
  ...
  if (idx !== -1) { bookings[idx].status = status; saveBookings(bookings); showToast('Booking status updated', 'success'); updateBookingsBadge(); }
}
...
function convertBookingToCustomer(id) {
  const b = getBookings().find(x => x.id === id);
  if (!b) return;
  const customers = getCustomers();
  if (customers.some(c => c.phone === b.phone || (b.email && c.email === b.email))) {
    showToast('Customer already exists in system', 'info'); return;
  }
  const customer = {
    id: Date.now(), name: b.name, phone: b.phone, email: b.email||'', reg: b.reg||'',
    make: b.make||'', model: b.model||'', year: b.year||'', motDue: '', notes: `Converted from booking ${b.ref}`, jobs: [], reminderLog: []
  };
  customers.push(customer);
  saveCustomers(customers);
  addNotification('info', `${b.name} added as customer from booking`, 'customers');
...
function sendBookingReminderWA(id) {
  ...
  const msg = `Hi ${firstName}, just confirming your ${b.service} booking at ${s.garageName||'Premier MOT'} on ${b.date} at ${b.time}. See you then! Any questions, call ${s.phone||'01234 567890'}.`;
...
// View toggle
document.getElementById('bookingsViewList')?.addEventListener('click', () => { dashBookingView='list'; document.getElementById('bookingCalendarWrap').style.display='none'; document.querySelector('.bookings-table-wrap').style.display=''; document.getElementById('bookingsViewList').classList.add('active'); document.getElementById('bookingsViewCal').classList.remove('active'); renderBookingsList(); });
document.getElementById('bookingsViewCal')?.addEventListener('click', () => { dashBookingView='calendar'; document.getElementById('bookingCalendarWrap').style.display=''; document.querySelector('.bookings-table-wrap').style.display='none'; document.getElementById('bookingsViewCal').classList.add('active'); document.getElementById('bookingsViewList').classList.remove('active'); renderBookingCalendar(); });
