...
function renderCurrentView() {
  if (dashBookingView === 'list') renderBookingsList();
...
function loadBookingsSection() {
  ...
  if (_bookingsUnsub) { renderCurrentView(); return; }
  ...
  _bookingsUnsub = garageRef('bookings')
    ...
    .onSnapshot(snap => {
      ...
      hideSpinner('page-bookings');
      updateBookingsBadge();
      if (window._currentSection === 'overview' && typeof window.loadOverview === 'function') window.loadOverview();
...
function setupBookingsViewToggles() {
  ...
  document.getElementById('calToday')?.addEventListener('click', () => { calCurrentDate = new Date(); renderBookingCalendar(); });
  document.getElementById('bookingSearch')?.addEventListener('input', e => { dashBookingSearch = e.target.value; renderCurrentView(); });
  document.getElementById('bookingStatusFilter')?.addEventListener('change', e => { dashBookingFilter = e.target.value; renderCurrentView(); });
  ...
  document.getElementById('printDaySheetBtn')?.addEventListener('click', () => printDaySheet());
}

function navigateCal(dir) {
  if (dashBookingView === 'month') calCurrentDate.setMonth(calCurrentDate.getMonth() + dir);
  else if (dashBookingView === 'week') calCurrentDate.setDate(calCurrentDate.getDate() + dir * 7);
  else if (dashBookingView === 'day') calCurrentDate.setDate(calCurrentDate.getDate() + dir);
...
function renderBookingCalendar() {
  ...
  if (dashBookingView === 'week')      renderWeekCalendar(container, todayStr);
  else if (dashBookingView === 'day')  renderDayCalendar(container, todayStr);
  else                                  renderMonthCalendar(container, todayStr);
}

function statusColor(status) {
  const map = {
    'Confirmed':'var(--red-light)','confirmed':'var(--red-light)',
    'Completed':'#22c55e','completed':'#22c55e','complete':'#22c55e','Complete':'#22c55e',
...
function matchBooking(b, ds) {
  ...
  if (dashBookingSearch) {
    const q = dashBookingSearch.toLowerCase();
    if (!(b.name||'').toLowerCase().includes(q) && !(b.reg||'').toLowerCase().includes(q)) return false;
  }
  return true;
}
...
function renderMonthCalendar(container, todayStr) {
  ...
  while (days.length % 7 !== 0) {
    ...
    const d = new Date(prev); d.setDate(prev.getDate() + 1);
    days.push({ date: d, inMonth: false });
  }
  ...
  for (let i = 0; i < days.length; i += 7) {
    ...
    rows.push(`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;margin-bottom:2px">
      ${week.map(({ date, inMonth }) => {
        ...
        const dayBookings = window._bookingsData.filter(b => matchBooking(b, ds)).sort((a, b) => (a.time||'').localeCompare(b.time||''));
        return `<div style="min-height:90px;background:${inMonth ? 'var(--dark-2)' : 'var(--dark-1)'};border:1px solid var(--border);border-radius:6px;padding:4px;${isToday ? 'border-color:var(--red-light)!important;' : ''}">
          ...
          ${dayBookings.map(b => `<div onclick="viewBookingModal('${b.id}')" style="cursor:pointer;font-size:0.65rem;padding:2px 4px;border-radius:3px;margin-bottom:2px;background:${statusColor(b.status)};color:#fff;overflow:hidden;white-space:nowrap;text-overflow:ellipsis" title="${esc(b.name)} — ${esc(b.service)}">${esc(b.time||'')} ${esc((b.name||'').split(' ')[0])}</div>`).join('')}
        </div>`;
      }).join('')}
    </div>`);
  }
...
function renderWeekCalendar(container, todayStr) {
  ...
  setCalTitle(weekStart.toLocaleDateString('en-GB', { day:'numeric', month:'short' }) + ' – ' + weekEnd.toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' }));

  const days = Array.from({ length: 7 }, (_, i) => { const nd = new Date(weekStart); nd.setDate(weekStart.getDate() + i); return nd; });
    ...
    ${days.map(day => {
      ...
      return `<div style="background:var(--dark-2);border:1px solid ${isToday ? 'var(--red-light)' : 'var(--border)'};border-radius:8px;overflow:hidden">
        ...
        <div style="padding:6px;min-height:120px">
          ${dayBookings.length === 0 ? '<div style="font-size:0.7rem;color:var(--text-dim);text-align:center;padding:12px 0">Free</div>' : dayBookings.map(b => `<div onclick="viewBookingModal('${b.id}')" style="cursor:pointer;margin-bottom:6px;padding:6px;border-radius:5px;background:${statusColor(b.status)};color:#fff"><div style="font-size:0.75rem;font-weight:700">${esc(b.time||'')}</div><div style="font-size:0.7rem">${esc((b.name||'').split(' ')[0])}</div><div style="font-size:0.65rem;opacity:0.8">${esc(b.service||'')}</div></div>`).join('')}
        </div>
      </div>`;
    }).join('')}
  </div>`;
...
function renderDayCalendar(container, todayStr) {
  ...
  const dayBookings = window._bookingsData.filter(b => matchBooking(b, ds)).sort((a, b) => (a.time||'').localeCompare(b.time||''));
  if (dayBookings.length === 0) { container.innerHTML = `<div style="text-align:center;padding:48px;color:var(--text-dim)"><i class="fas fa-calendar-day" style="font-size:2rem;margin-bottom:12px;display:block"></i>No bookings for this day.</div>`; return; }
  container.innerHTML = `<div style="display:flex;flex-direction:column;gap:10px">
    ${dayBookings.map(b => `<div onclick="viewBookingModal('${b.id}')" style="cursor:pointer;display:grid;grid-template-columns:80px 1fr auto;gap:16px;align-items:center;background:var(--dark-2);border:1px solid var(--border);border-left:4px solid ${statusColor(b.status)};border-radius:8px;padding:14px 16px">
...
async function deleteBooking(id) {
  ...
  try { await fsDel('bookings', id); showToast('Booking deleted', 'info'); }
...
function bookingStatusBadge(s) {
  const map = {
    ...
    'Completed':'badge-completed','completed':'badge-completed','complete':'badge-completed','Complete':'badge-completed',
    'Cancelled':'badge-cancelled','cancelled':'badge-cancelled',
    'No Show':'badge-contacted','no_show':'badge-contacted',
    'In Progress':'badge-booked','in_progress':'badge-booked',
    'Pending':'badge-new','pending':'badge-new'
  };
...
function printDaySheet() {
  ...
  const todays = window._bookingsData.filter(b => b.date === ds && b.status !== 'Cancelled').sort((a, b) => (a.time||'').localeCompare(b.time||''));
  const w = window.open('', '_blank');
  w.document.write(`<html><head><title>Day Sheet ${ds}</title><style>body{font-family:sans-serif;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:8px 12px;text-align:left}th{background:#f0f0f0}h2{margin:0 0 16px}</style></head><body><h2>Day Sheet — ${ds}</h2><table><thead><tr><th>Time</th><th>Customer</th><th>Reg</th><th>Service</th><th>Phone</th><th>Status</th></tr></thead><tbody>${todays.map(b=>`<tr><td>${b.time||'—'}</td><td>${b.name||'—'}</td><td>${b.reg||'—'}</td><td>${b.service||'—'}</td><td>${b.phone||'—'}</td><td>${b.status||'—'}</td></tr>`).join('')}</tbody></table></body></html>`);
  w.print();
}
